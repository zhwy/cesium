  ...
  extends Cesium.QuadtreePrimitive
{
  ...
  renderTiles(frameState) {
    ...
    //   not yet GPU-ready. Updating with show=false still advances Cesium's
    //   asynchronous geometry pipeline (the show check happens after
    //   loadAsynchronous) without emitting draw commands, so the tile becomes
    //   drawable in a later frame and is then promoted atomically.
