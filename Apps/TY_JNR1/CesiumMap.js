...
var CesiumMap = (function() {
    ...
    var _initRoam = function() {
        ...
        var keydown = function(e) {
            if (_roamMode) {
                switch (e.keyCode) {
                    case 'W'.charCodeAt(0):
                        _viewer.clock.multiplier =
                            _roamVelocity * cameraDirection;
                        break;
                    case 'S'.charCodeAt(0):
                        _viewer.clock.multiplier =
                            -_roamVelocity * cameraDirection;
            ...
            if (_roamMode) {
                switch (e.keyCode) {
                    ...
                    case 'R'.charCodeAt(0): //r
                        ...
                        var dx =
                            cartographic2.latitude - cartographic1.latitude;
                        var dy =
                            cartographic2.longitude - cartographic1.longitude;
        ...
        var trackHandler = new Cesium.ScreenSpaceEventHandler(
            ...
            if (scene.mode !== Cesium.SceneMode.MORPHING && !_roamMode) {
                ...
                if (
                    ...
                    if (Cesium.defined(_roadLine) && _rightStopTime) {
                        ...
                        var si = 0;
                        var ei =
                            _rightStopTime.secondsOfDay -
                            _startTime.secondsOfDay;
                        var ti = -1;
                        while (si < ei) {
                            ...
                            if (
                                (cartesian.y - midPos0.y) *
                                    (cartesian.y - midPos1.y) <=
                                0
                            ) {
                                ti = mid;
                                break;
        ...
        _viewer.clock.onTick.addEventListener(function(clock) {
            //调整相机位置
            var loc = Cesium.Cartographic.fromCartesian(
            ...
            if (_roamMode) {
                var seconds =
                    clock.currentTime.secondsOfDay - _startTime.secondsOfDay;
                var side = _directInfo == '上行' ? 'right' : 'left';
                var position = _roamPath.position
                            ...
                            if (
                                ...
                                if (Cesium.defined(_selectModel)) {
                                    ...
                                    if (_selectModel != _clickModel) {
                                        if (
                                            Cesium.defined(_selectModel.color)
                                        ) {
                                            _selectModel.color = new Cesium.Color(
                                                1,
                                                1,
                                                1,
                                                1
    ...
    var _initRoadModel = function(data) {
        ...
        jgModel.loadProgress.addEventListener(function(
            ...
            if (_roamMode) {
                ...
                var total =
                    _progress.JG + _progress.SB + _progress.MQ + _progress.ZX;
                if (total > 99) {
                    total = 99;
                }
                            ...
                            try {
                                if (Cesium.defined(pickedObject.tileset)) {
                                    if (
                                        ...
                                        var name = pickedObject.getProperty(
                                            'name'
                                        );
                                        document.getElementById(
                                            _options.domId
                                        ...
                                        if (Cesium.defined(_clickModel)) {
                                            if (
                                                ...
                                                _clickModel.color = new Cesium.Color(
                                                    1,
                                                    1,
                                                    1,
                                                    1
    ...
    _cesiumMap.prototype = {
        ...
        Locating: function(name) {
            ...
            //if (!_roamMode) {
            _flyToHome();
            $.getJSON(_options.dataServerUrl + '/locations.json', function(
                result
            ) {
                if (name in result) {
                    _viewer.camera.flyTo({
