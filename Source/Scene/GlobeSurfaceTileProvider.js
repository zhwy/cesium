...
function GlobeSurfaceTileProvider(options) {
  ...
  this.cutPolygonUniformMap = undefined;
  this._cutPolygonUniformMap = undefined;
...
GlobeSurfaceTileProvider.prototype.beginUpdate = function(frameState) {
  ...
  if (defined(multiClippingPlanes) && multiClippingPlanes.length > 0) {
    ...
    if (!dontUpdateTexture) {
      var arrayBuffer = useFloatTexture ? new Float32Array(widthTotal * height * 4) : new Uint8Array(widthTotal * height * 4);
...
function createTileUniformMap(frameState, globeSurfaceTileProvider) {
  var uniformMap = {
    ...
    u_multiClippingPlanesLength: function() {
      var clippingPlanesLength = globeSurfaceTileProvider._multiClippingPlanesLengthTexture;
...
function addDrawCommandsForTile(tileProvider, tile, frameState) {
  ...
  // multipolygon terrain clipping
  var cutPolygonUniformMapChanged =
    tileProvider._cutPolygonUniformMap !== tileProvider.cutPolygonUniformMap;
  if (cutPolygonUniformMapChanged) {
    tileProvider._cutPolygonUniformMap = tileProvider.cutPolygonUniformMap;
    var drawCommandsLength = tileProvider._drawCommands.length;
    for (var i = 0; i < drawCommandsLength; ++i) {
      tileProvider._uniformMaps[i] = createTileUniformMap(
  ...
  do {
    ...
    surfaceShaderSetOptions.multiClippingPlanes = tileProvider._multiClippingPlanes;
